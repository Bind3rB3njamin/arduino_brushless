...
//////////////////////////////////////////////
//  Configuration of motion Module
//////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////
//  Command format
//  #[_v_elocity/_a_ngle][_s_et {value}/_g_et/_o_utputmode[0(polling)/1(streaming)]
//  example getVelocity:          #vg
//  example setVelocity:          #vs 100.0  (answer: #v 100.0)
//  example setStreamingOutput:   #vo1
/////////////////////////////////////////////////////////////////////////////////////
...
#define OUTPUT_MODE_CHAR          'o'
#define OUTPUT_MODE_POLL          '0'
#define OUTPUT_MODE_STREAM        '1'
#define ODOMETRY_UPDATE_STEPS     10   //ms

#include <BufferedUART.h>

unsigned long odometry_timestamp_old = 0;
unsigned long odometry_timestamp = 0;
boolean stream_odometry = false;
boolean stream_velocity = false;
boolean print_velocity = false;
boolean stream_servo_angle = false;
...
char readChar()
{
  while (BufferedUART::bufferedLength() < 1) { } // Block
  return BufferedUART::read_c();
}
...
void loop()
{
  ...
  if (BufferedUART::bufferedLength() >= 2)
  {
    ...
    {
      int command = BufferedUART::read_c(); // Commands
      ...
      else if (command == 's') // _s_ynch request
      {
        ...
        BufferedUART::write_s("#SYNCH");
        BufferedUART::write_s(id, 2);
        BufferedUART::write_c('\n');
      }
      else if (command == VELOCITY_COMAND_CAHR)
      {
        ...
        else if(mode==OUTPUT_MODE_CHAR)
        {
          char streammode = BufferedUART::read_c();
          if(streammode == OUTPUT_MODE_POLL)
            stream_velocity = false;
          if(streammode == OUTPUT_MODE_STREAM)
            stream_velocity = true;
        }
      }
      ...
      {  
        ...
        else if(mode==OUTPUT_MODE_CHAR)
        {
          char streammode = BufferedUART::read_c();
          if(streammode == OUTPUT_MODE_POLL)
            stream_servo_angle = false;
          if(streammode == OUTPUT_MODE_STREAM)
            stream_servo_angle = true;
        }      
      }
      else if (command == 'o') // Set _o_utput mode
      {
        ...
        else if (output_param == 'e') // _e_rror output settings
        {
          ...
          {
            BufferedUART::write_s("#AMG-ERR:");
            BufferedUART::write_i(num_accel_errors); BufferedUART::write_c(',');
            BufferedUART::write_i(num_magn_errors); BufferedUART::write_c(',');
            BufferedUART::write_i(num_gyro_errors); BufferedUART::write_c('\n');
          }
        }
      }
      else if (command == 'p') // Set _p_rint calibration values
      {
         BufferedUART::write_s("ACCEL_X_MIN:");BufferedUART::write_f(ACCEL_X_MIN,2);BufferedUART::write_c('\n');
         BufferedUART::write_s("ACCEL_X_MAX:");BufferedUART::write_f(ACCEL_X_MAX,2);BufferedUART::write_c('\n');
         BufferedUART::write_s("ACCEL_Y_MIN:");BufferedUART::write_f(ACCEL_Y_MIN,2);BufferedUART::write_c('\n');
         BufferedUART::write_s("ACCEL_Y_MAX:");BufferedUART::write_f(ACCEL_Y_MAX,2);BufferedUART::write_c('\n');
         BufferedUART::write_s("ACCEL_Z_MIN:");BufferedUART::write_f(ACCEL_Z_MIN,2);BufferedUART::write_c('\n');
         BufferedUART::write_s("ACCEL_Z_MAX:");BufferedUART::write_f(ACCEL_Z_MAX,2);BufferedUART::write_c('\n');
         BufferedUART::write_c('\n');
         BufferedUART::write_s("MAGN_X_MIN:");BufferedUART::write_f(MAGN_X_MIN,2);BufferedUART::write_c('\n');
         BufferedUART::write_s("MAGN_X_MAX:");BufferedUART::write_f(MAGN_X_MAX,2);BufferedUART::write_c('\n');
         BufferedUART::write_s("MAGN_Y_MIN:");BufferedUART::write_f(MAGN_Y_MIN,2);BufferedUART::write_c('\n');
         BufferedUART::write_s("MAGN_Y_MAX:");BufferedUART::write_f(MAGN_Y_MAX,2);BufferedUART::write_c('\n');
         BufferedUART::write_s("MAGN_Z_MIN:");BufferedUART::write_f(MAGN_Z_MIN,2);BufferedUART::write_c('\n');
         BufferedUART::write_s("MAGN_Z_MAX:");BufferedUART::write_f(MAGN_Z_MAX,2);BufferedUART::write_c('\n');
         BufferedUART::write_c('\n');
         BufferedUART::write_s("MAGN_USE_EXTENDED:");
         if (CALIBRATION__MAGN_USE_EXTENDED) 
         {
           BufferedUART::write_s("true");
           BufferedUART::write_c('\n');
         }
         else
         {
           BufferedUART::write_s("false");
           BufferedUART::write_c('\n');
         }
         BufferedUART::write_s("magn_ellipsoid_center:[");BufferedUART::write_f(magn_ellipsoid_center[0],4);BufferedUART::write_c(',');
         BufferedUART::write_f(magn_ellipsoid_center[1],4);BufferedUART::write_c(',');
         BufferedUART::write_f(magn_ellipsoid_center[2],4);BufferedUART::write_s("]\n");
         BufferedUART::write_s("magn_ellipsoid_transform:[");
         for(int i = 0; i < 3; i++){
           BufferedUART::write_s("[");
           for(int j = 0; j < 3; j++){
             BufferedUART::write_f(magn_ellipsoid_transform[i][j],7);
             if (j < 2) BufferedUART::write_s(",");
           }
           BufferedUART::write_s("]");
         ...
         BufferedUART::write_s("]\n");
         BufferedUART::write_c('\n'); 
         BufferedUART::write_s("GYRO_AVERAGE_OFFSET_X:");BufferedUART::write_f(GYRO_AVERAGE_OFFSET_X,2);BufferedUART::write_c('\n');
         BufferedUART::write_s("GYRO_AVERAGE_OFFSET_Y:");BufferedUART::write_f(GYRO_AVERAGE_OFFSET_Y,2);BufferedUART::write_c('\n');
         BufferedUART::write_s("GYRO_AVERAGE_OFFSET_Z:");BufferedUART::write_f(GYRO_AVERAGE_OFFSET_Z,2);BufferedUART::write_c('\n');
      }
      else if (command == 'c') // Set _i_nput mode
      {
        ...
        if (input_param == 'a')  // Calibrate _a_ccelerometer
        {
          ...
          float value_param = BufferedUART::read_f();
  ...
  if((millis() - odometry_timestamp) >= ODOMETRY_UPDATE_STEPS)
  {
    odometry_timestamp_old = odometry_timestamp;
    odometry_timestamp = millis();
    updateOdometry(odometry_timestamp - odometry_timestamp_old);
  }   
  ...
  if((millis() - timestamp) >= OUTPUT__DATA_INTERVAL)
  {
    ...
    {
      printOdometryMsg();
      print_servo_angle = stream_servo_angle;
      print_velocity = stream_velocity;
    }
    ...
    BufferedUART::write_s("loop time (ms) = ");
    BufferedUART::write_i(millis() - timestamp);
#endif
  }
#if DEBUG__PRINT_LOOP_TIME == true
  else
  {
    BufferedUART::write_s("waiting...\n");
  }
#endif
}
