...
void Read_Accel()
{
  ...
  {
    num_accel_errors++;
    if (output_errors) BufferedUART::write_s("!ERR: reading accelerometer\n");
  }
#endif
#if HW__BRAND == HW__BRAND_POLOLU  
  compass.readAcc();
...
// 9DOF Sensor Stick SEN-10724 using HMC5883L magnetometer
  ...
  {
    num_magn_errors++;
    if (output_errors) BufferedUART::write_s("!ERR: reading magnetometer\n");
  }
#endif
#if HW__BRAND == HW__BRAND_POLOLU  
  compass.readMag();
...
void Read_Gyro()
{
  ...
  {
    num_gyro_errors++;
    if (output_errors) BufferedUART::write_s("!ERR: reading gyroscope\n");
  }
#endif
#if HW__BRAND == HW__BRAND_POLOLU  
  gyroscope.read();
